...
export type DragOptions = {
     ...
     * CSS pixel ("px") units are assumed unless otherwise specified.
     *
     */
...
export const draggable = (node: HTMLElement, options: DragOptions = {}) => {
    ...
    let translateX = isString(defaultPosition.x) ? cssUnitsToPixels(defaultPosition.x, node) : defaultPosition.x,
        translateY = isString(defaultPosition.y) ? cssUnitsToPixels(defaultPosition.y, node) : defaultPosition.y;
    ...
    function setTranslate(xPos: number | string = translateX, yPos: number | string = translateY) {
        const validUnits = ["%", "cm", "mm", "Q", "in", "pc", "pt", "px",
            "em", "ex", "ch", "rem", "lh", "rlh", "vw", "vh",
            "vmin", "vmax", "vb", "vi", "svw", "svh", "lvw", "lvh",
            "dvw", "dvh"];

        xPos = isString(xPos) ? xPos.trim().toLowerCase() : xPos;
        yPos = isString(yPos) ? yPos.trim().toLowerCase() : yPos;

        let xValue, yValue;

        if (isString(xPos) && xPos.startsWith('calc') && xPos.endsWith(')')) {
            xValue = xPos;
        } else {
            let matches = isString(xPos) ? xPos.match(/([-+]?\d*\.?\d+)([a-z%]*)/i) : null;
            let num, xUnit;
            if (matches) {
                num = matches[1];
                xUnit = matches[2] || "px";
            ...
            if (!validUnits.includes(xUnit)) throw new Error(`Invalid X unit: ${xUnit}`);
            xValue = `${num}${xUnit}`;
        }


        if (isString(yPos) && yPos.startsWith('calc') && yPos.endsWith(')')) {
            yValue = yPos;
        } else {
            let matches = isString(yPos) ? yPos.match(/([-+]?\d*\.?\d+)([a-z%]*)/i) : null;
            let num, yUnit;
            if (matches) {
                num = matches[1];
                yUnit = matches[2] || "px";
            ...
            if (!validUnits.includes(yUnit)) throw new Error(`Invalid Y unit: ${yUnit}`);
            yValue = `${num}${yUnit}`;
        }

        if (!transform) {
            let common = `${xValue}, ${yValue}`;
            return setStyle(
                node,
                'transform',
    ...
    function cssUnitsToPixels(value: string, element: HTMLElement = document.body): number {
        // Extracting the absolute value and sign
        const isNegative = value.startsWith('-');
        const absoluteValue = isNegative ? value.slice(1) : value;

        // Create a temporary div and set the width to the absolute value
        const tempElem = document.createElement('div');
        tempElem.style.width = absoluteValue;
        tempElem.style.visibility = "hidden"; // So it doesn't affect your layout
        ...
        // Clean up
        element.removeChild(tempElem);

        // Apply the sign to the pixel value
        return isNegative ? -pixelValue : pixelValue;
    }
    ...
    function dragStart(e: PointerEvent) {
        ...
        if (canMoveInX) {
            const xOffsetInPixels = isString(xOffset) ? cssUnitsToPixels(xOffset, node) : xOffset;
            initialX = clientX - xOffsetInPixels / inverseScale;
        }
        if (canMoveInY) {
            const yOffsetInPixels = isString(yOffset) ? cssUnitsToPixels(yOffset, node) : yOffset;
            initialY = clientY - yOffsetInPixels / inverseScale;
        }
